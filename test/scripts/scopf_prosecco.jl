...
# # data parsing
# data = PowerModels.parse_file("test/data/prosecco_base.m")
...
# N-1 
for idx = 1:19 # idx [1:10] for converter outage, idx [11:19] for dc branch outage
    nw = idx + 1
    if idx <= 11
        data_all["nw"]["$nw"]["convdc"]["$idx"]["status"] = 0
    else
        data_all["nw"]["$nw"]["branchdc"]["$(idx-10)"]["status"] = 0
    end
    # data_all["nw"]["$nw"]["convdc"]["$idx"]["status"] = 0
...
for (n, network) in result["solution"]["nw"]
    for (g, gen) in network["gen"]
        ...
        qg[parse(Int, n), parse(Int, g)] = gen["qg"] * data_all["nw"]["1"]["baseMVA"]
    end
    for (c, conv) in network["convdc"]
        pconv[parse(Int, n), parse(Int, c)] = conv["pdc"] * data_all["nw"]["1"]["baseMVA"]
        qconv[parse(Int, n), parse(Int, c)] = conv["qconv"] * data_all["nw"]["1"]["baseMVA"]
    end
    for (l, load) in network["load"]
        pd[parse(Int, n), parse(Int, l)] = load["pflex"] * data_all["nw"]["1"]["baseMVA"]
    end
end
