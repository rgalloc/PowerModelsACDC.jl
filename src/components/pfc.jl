...
function variable_pfc_current(pm::_PM.AbstractIVRModel; nw::Int=_PM.nw_id_default, bounded::Bool=true, report::Bool=true)
    ...
    #report && _PM.sol_component_value_edge(pm, nw, :pfc, :ipfc_dc, _PM.ids(pm, nw, :pfc), ipfc_dc) # Check later how to fix it.
    #report && _PM.sol_component_value_edge(pm, nw, :pfc, :pfc_current_fr, :pfc_current_to, _PM.ref(pm, nw, :arcs_pfc_from), _PM.ref(pm, nw, :arcs_pfc_to), ipfc_dc)
...
function constraint_duty_cycle_pfc(pm, nw, i, arcs_pfc, pfc_terminal_1, pfc_terminal_2, pfc_terminal_3)
    ...
    # JuMP.@constraint(pm.model, (1-duty_cycle)*abs(I3) - duty_cycle*abs(I2) == 0) 
    JuMP.@constraint(pm.model, c_voltage*((1-duty_cycle)*I3 - duty_cycle*I2) == 0) ## Changed the constraint ## Added the e variable
    # JuMP.@constraint(pm.model, c_voltage*((1-duty_cycle)*abs(I3) - duty_cycle*abs(I2)) == 0)  ## Changed the constraint ## Added the e variable
end
